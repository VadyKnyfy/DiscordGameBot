...
import static discord4j.gateway.intent.Intent.GUILD_MEMBERS;
import static discord4j.gateway.intent.Intent.GUILD_PRESENCES;


public class App {
    static int type= -1;
    static HashMap<String,GSession> sessions = new HashMap<String,GSession>();
    ...
    public static void playgame(MessageCreateEvent event, String[] messages){
        ...
        if(sessions.get(scode).joinBoth) {
            String activityname = GActivity.getActivitybyId(Integer.valueOf(messages[1]));
            ...
            if (autm.equalsIgnoreCase(sessions.get(scode).p1m.getMention())) {
                ...
                String ready1 = sessions.get(messages[2]).p1r==true ? "ready :green_circle:" : "not ready :red_circle:";
                String ready2 = sessions.get(messages[2]).p2r==true ? "ready :green_circle:" : "not ready :red_circle:";

                sessions.get(messages[2]).mainMessage.edit(msg->{
                    msg.setContent("Game code: "+
                            messages[2]+"\tRound: "+String.valueOf(sessions.get(messages[2]).round)+
                            "\nPlayer1\t\tPlayer2\n"+
                            ...
                            "\nHp:"+String.valueOf(sessions.get(messages[2]).p1.getCharacter().getHealthpoint())+
                            "\t\tHp:"+String.valueOf(sessions.get(messages[2]).p2.getCharacter().getHealthpoint())+
                            "\n"+ready1
                            +"\t\t"+
                            ready2);
                }).block();
    ...
    public static void  joingame(MessageCreateEvent event, String[] messages){
        if((sessions.get(messages[2]).connectedBoth)&&(!sessions.get(messages[2]).joinBoth)){
            ...
            if(autm.equalsIgnoreCase(sessions.get(messages[2]).p1m.getMention())){
                ...
                String join1 = sessions.get(messages[2]).p1j==true ? "join :green_circle:" : "not join :red_circle:";
                String join2 = sessions.get(messages[2]).p2j==true ? "join :green_circle:" : "not join :red_circle:";
                sessions.get(messages[2]).mainMessage.edit(msg->{
